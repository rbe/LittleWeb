...
module SecureAccess
  ...
  class AccessRequestController < HTTP::Controller
    ...
    def process
      return @response.bad_request_response unless @request.query_value? 'token', 'hash'

      token = create_token
      return render unless @messages.empty?
    ...
    def create_token
      ...
      add_message 'No access to URL' unless token.access? user, url
      token
    end

    def validate_url
      url = @request.request_uri
      add_message 'Invalid URL' unless url =~ %r{[A-Za-z0-9./]+}
      url
    end

    def validate_user
      user = @request.query_value 'user'
      add_message 'Invalid user' unless user =~ URI::MailTo::EMAIL_REGEXP
      user
    end
